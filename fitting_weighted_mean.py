...
ckpt_path = "ckpt/weighted_mean_23ML"
ckpt_name = "weighted_mean_23ML"
...
class VSModel(nn.Module):
    def __init__(self, batter, pitcher, versus):
        ...
        self.v1 = nn.Parameter(torch.Tensor(list(batter["average"])))
        self.v2 = nn.Parameter(torch.Tensor(list(pitcher["average"])))
    ...
    def forward(self):
        ...
        for i in range(len(self.versus)):
            ...
            prob = self.weight*(self.v1[batter_index])+(1-self.weight)*(self.v2[pitcher_index])
